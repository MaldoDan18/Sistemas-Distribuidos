...
def notify_broker_event(broker_config, event_type, sale_id, **extra_args):
    if not broker_config or not broker_config.get("host"):
        return

    command = [
        "java",
        "-cp",
        broker_config["java_cp"],
        "broker.BrokerEventCli",
        "--host",
        broker_config["host"],
        "--port",
        str(broker_config["port"]),
        "--bind",
        broker_config["bind"],
        "--event",
        event_type,
    ...
    if event_type == "SERVER_REGISTERED":
        command.extend([
            "--server-host",
            str(extra_args.get("server_host", "127.0.0.1")),
            "--server-port",
            str(int(extra_args.get("server_port", 0))),
            "--expected-clients",
            str(int(extra_args.get("expected_clients", 0))),
        ])
    elif event_type == "CLIENT_CONNECTED":
        command.extend([
            "--client-id",
            str(extra_args.get("client_id", "")),
            "--buyers",
            str(int(extra_args.get("buyers", 0))),
        ])
    elif event_type == "SERVER_FINISHED":
        command.extend([
            "--summary",
...
def monitor_sold_out(ticket_state, coordinator_client=None, broker_config=None, sale_id=None):
    ...
    if sale_id:
        summary_text = (
            f"sold={finish_summary['sold_count']}/{finish_summary['total_seats']}"
            f", reason={finish_summary['close_reason']}"
        )
        notify_broker_event(broker_config, "SERVER_FINISHED", sale_id, summary=summary_text)
...
def main():
    ...
    broker_config = {
        ...
        "bind": args.broker_bind,
        "java_cp": args.broker_java_cp,
    }
    ...
    monitor_thread = threading.Thread(
        target=monitor_sold_out,
        args=(ticket_state, coordinator_client, broker_config, sale_id),
        daemon=True,
    )
