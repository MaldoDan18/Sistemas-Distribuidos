...
class CoordinatorState:
    ...
    def broadcast_global_start_delayed(self, countdown_seconds=8):
        """Broadcast global start after a countdown with printed feedback."""
        def _countdown_and_broadcast():
            print(f"\n[Coordinador] Iniciando cuenta regresiva de {countdown_seconds} segundos antes de GLOBAL_START...")
            for i in range(countdown_seconds, 0, -1):
                print(f"[Countdown] {i}...")
                time.sleep(1)
            print("[Countdown] 0 - ¡INICIANDO VENTAS GLOBALES!")
            self.broadcast_global_start()

        # Start countdown in a separate thread to not block the server
        self.broadcast_thread = threading.Thread(target=_countdown_and_broadcast, daemon=True)
